...
async function init() {
  try {
    ...
    alert(
      "Error loading parish data. Please make sure rocor_parishes.json is in the same directory.",
    );
  }
}
...
function createPopupContent(parish) {
  const location = [parish.city, parish.state, parish.country].filter(Boolean).join(", ");
